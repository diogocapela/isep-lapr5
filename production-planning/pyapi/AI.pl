...
:- dynamic maq_linha/2.
:- dynamic maq_tipo_maquina/2.
:- dynamic tipo_operacoe/1.
...
insereOperacaoProdutoEmAgenda(Operacao, Produto, Cliente, NumeroUnidades, Tarefa, Linha, TempoAtual, TempoApos):-
	...
	findall(TipoMaquinaTemp,
		operacao_maquina(TipoOperacao,TipoMaquinaTemp,Ferramenta,TempoSetup,TempoExecucao), 
		ListaTipoMaquinas
		),
	find_maquina_linha(ListaMaquinasLinha, ListaTipoMaquinas, ListaTipoMaquinas, Maquina),
...
find_maquina_linha([_|Resto], [], TipoMaquinas, Maquina):-
	find_maquina_linha(Resto, TipoMaquinas, TipoMaquinas, Maquina), !.

find_maquina_linha([MaquinaAtual|Resto], [TipoMaquinaAtual|RestoTipo], TiposMaquinas, Maquina):-
	(
		(maq_tipo_maquina(MaquinaAtual, TipoMaquinaAtual),
		Maquina = MaquinaAtual, !);
		(find_maquina_linha([MaquinaAtual|Resto], RestoTipo, TiposMaquinas, Maquina))
	).
